export const calcTimeAgo = (time) => {
  ...
  const category = (ago, name) => {
    const timeAgo = ago < 2 ? ago + ` ${name} ago ` : ago + ` ${name} ago `;
    return timeAgo;
  };
  if (secondsAgo < 60) {
    return category(secondsAgo, "second ");
  } else if (minutesAgo < 60) {
    return category(minutesAgo, "minute ");
  } else if (hoursAgo < 24) {
    return category(hoursAgo, "hours ");
  } else if (daysAgo < 7) {
    return category(daysAgo, "day");
  } else {
    return category(weeksAgo, "week");
  }
};
